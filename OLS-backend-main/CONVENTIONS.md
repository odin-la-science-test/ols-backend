...
| `history` | Systeme | Historique persistant undo/redo (enregistrement automatique via `CrudActionEvent`) |
...
| `SoftDeletable` | Interface pour le soft delete (`getDeletedAt`, `setDeletedAt`, `isDeleted`). **Implemente par** tous les CRUD owned : Contact, Note, Annotation, StudyCollection, SupportTicket, SharedItem. Le `delete()` dans `AbstractOwnedCrudService` fait un soft delete automatique pour les entites `SoftDeletable` |
...
| `CrudActionEvent` | — | history (enregistrement automatique). Publie par `AbstractOwnedCrudService` apres chaque create/update/delete. Header `X-History-Skip` empeche l'enregistrement pendant undo/redo |
...
- JWT (access + refresh tokens) via cookies httpOnly (Secure, SameSite=Lax), generation JJWT 0.12.6
...
- CORS : localhost:3000 (dev), odinlascience.fr (prod)
