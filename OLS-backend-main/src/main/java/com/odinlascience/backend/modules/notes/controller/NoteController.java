...
public class NoteController extends AbstractOwnedCrudController<NoteDTO, CreateNoteRequest, UpdateNoteRequest, NoteService> {
    ...
    public ResponseEntity<List<NoteDTO>> search(@RequestParam("query") @NotBlank @Size(max = 200) String query, Authentication auth) {
    ...
    public ResponseEntity<List<NoteDTO>> searchByTag(
            @RequestParam("query") @NotBlank @Size(max = 200) String tag,
            Authentication auth
    ) {
