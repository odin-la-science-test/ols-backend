...
public class SupportTicketAdminService {
    ...
    public List<SupportTicketDTO> getAllTickets() {
        return repository.findAllByDeletedAtIsNullOrderByCreatedAtDesc().stream().map(mapper::toDTO).toList();
    }
    ...
    public TicketStatsDTO getStats() {
        return TicketStatsDTO.builder()
                ...
                .open(repository.countByStatusAndDeletedAtIsNull(TicketStatus.OPEN))
                .inProgress(repository.countByStatusAndDeletedAtIsNull(TicketStatus.IN_PROGRESS))
                .resolved(repository.countByStatusAndDeletedAtIsNull(TicketStatus.RESOLVED))
                .closed(repository.countByStatusAndDeletedAtIsNull(TicketStatus.CLOSED))
                .build();
    }
