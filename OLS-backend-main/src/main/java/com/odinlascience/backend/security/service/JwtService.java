...
public class JwtService {
    ...
    public String generateAccessToken(UserDetails userDetails, UUID sessionId) {
        ...
        return buildToken(claims, userDetails, accessTokenExpiration, "access");
    }
    ...
    public UUID extractSessionIdIgnoringExpiration(String token) {
        try {
            return extractSessionId(token);
        } catch (ExpiredJwtException e) {
            String sessionId = (String) e.getClaims().get("sessionId");
            return sessionId != null ? UUID.fromString(sessionId) : null;
        }
    }
